 ...
 * DES加密解密算法
 * @author yangwenjian
 * @serial   http://hkllzh.iteye.com/blog/1295245
 * @version V1.0.0
 */
public class DESPlus {
	private static String strDefaultKey = PropertiesUtil.getProperty("DESP_KEY", null);
	...
	public static byte[] hexStr2ByteArr(String strIn) {
		byte[] arrB = null;
		try {
			arrB = strIn.getBytes(UNICODE_FORMAT);
		} catch (Exception e) {
			e.printStackTrace();
			return arrB;
		}
	...
	public String encrypt(String strIn) {
		byte[] bytes = null;
		try {
			bytes = strIn.getBytes(UNICODE_FORMAT);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return byteArr2HexStr(encrypt(bytes));
	}
	...
	public String decrypt(String strIn) throws UnsupportedEncodingException {
		return new String(decrypt(hexStr2ByteArr(strIn)), UNICODE_FORMAT);
	}
